...
namespace SCERSE
{
    ...
    MainWindow::MainWindow(QWidget *parent)
        : QMainWindow(parent), codeEditor(nullptr), errorTable(nullptr), suggestionsList(nullptr), mainSplitter(nullptr), analyzeTimer(nullptr), statusLabel(nullptr), lineColLabel(nullptr), errorCountLabel(nullptr), isModified(false)
    {
                                   ...
                                   const std::vector<std::pair<std::string, std::string>> &syntaxErrors)
    {
        ...
        for (const auto &err : lexErrors)
        {
            ...
            lineItem->setForeground(QColor(180, 90, 255)); // Light Purple
            msgItem->setForeground(QColor(180, 90, 255));
        ...
        for (const auto &[err, sug] : syntaxErrors)
        {
            ...
            bool isWarning = errStr.startsWith("Warning");

            // Warnings → Yellow 
            if (isWarning)
            {
                lineItem->setForeground(QColor(255, 215, 0)); // Yellow
                msgItem->setForeground(QColor(255, 215, 0));
            }
            // Errors → Red
            else
            {
